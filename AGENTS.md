...
このリポジトリは、画像をブラウザ内だけで変換・圧縮する静的 React + TypeScript + Vite アプリである。Source of truth は executable code/config/tests > docs とし、README.md は product/scope claims、docs/privacy.md は user-facing privacy policy、package.json は script composition を所有する。

## まず読む場所

- Architecture、module seam、runtime contract、image transform、Worker、metadata、technical privacy boundary、その evidence: [docs/architecture.md](docs/architecture.md)
...
- Product と scope の文言: [README.md](README.md)。主張は実装と E2E に trace する。
- User-facing privacy policy: [docs/privacy.md](docs/privacy.md)
...
- protocol、MIME、metadata、privacy の契約を変える場合は、policy、technical contract、実装・unit test・live E2E evidence を同じ変更として扱う。
...
- [ ] contract/product/privacy claims が変わったら owner docs を更新し、README の product/scope claims を code/E2E に trace した。
