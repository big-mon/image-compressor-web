...
- ソース、プレビュー、出力はメモリ上の値とブラウザが管理するオブジェクト／オブジェクト URL を使います。入力の差し替え、画面の破棄などでは、アプリが管理するオブジェクト URL などを解放します。ただし、ブラウザや OS の一般的なキャッシュや一時保存の有無を、本アプリが制御・保証するものではありません。
