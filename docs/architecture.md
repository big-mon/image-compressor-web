...
画像のeditor-columnはviewport全体に固定し、編集ステージと比較ステージはその全面を使う。画像は縦横比を保って全体表示し、上下のUIの高さを画像のfit計算から差し引かない。保存・view切替・編集ツールは独立した前面レイヤーに置く。右下ハンドルは全画面fitした枠の右下を基準に配置し、44pxの操作領域が画面外や上部ツールバーや編集・比較切替に重ならないようCSSのclampで制限する。
