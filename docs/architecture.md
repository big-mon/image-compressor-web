...
クロップはアスペクト比プリセットと画像上の移動・リサイズで操作し、三分割線を固定表示する。キーボードでは枠の矢印キーを移動、右下ハンドルの矢印キーをサイズ変更に割り当て、Shiftで変更量を1pxから10pxへ増やす。ハンドルのキーイベントは親の移動操作へ伝播させない。座標・ズーム・パンの詳細入力UIと補助線の切替は設けない。
